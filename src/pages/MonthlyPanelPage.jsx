    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-lg md:text-xl">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden card-lg">
        <div className="px-6 py-5 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-50">{isEn ? 'Summary table' : 'Tabla resumen'}</h2>
          <span className="text-lg text-gray-500 dark:text-slate-300">{filteredJobs.length} {isEn ? 'records' : 'registros'}</span>
        </div>
        <div className="">
          <table className="min-w-full text-lg md:text-xl text-left">
            <thead className="bg-gray-50 dark:bg-slate-800 text-gray-700 dark:text-slate-100 uppercase font-semibold border-b border-gray-200 dark:border-slate-700">
              <tr>
                <th className="px-4 md:px-6 py-4">{t('monthlyPage.columns.date')}</th>
                <th className="px-4 md:px-6 py-4">{t('monthlyPage.columns.description')}</th>
                <th className="px-4 md:px-6 py-4">{t('monthlyPage.columns.location')}</th>
                <th className="px-4 md:px-6 py-4">{t('monthlyPage.columns.worker')}</th>
                <th className="px-4 md:px-6 py-4">{t('monthlyPage.columns.type')}</th>
                <th className="px-4 md:px-6 py-4">{t('monthlyPage.columns.group')}</th>
                <th className="px-4 md:px-6 py-4 text-right">{t('monthlyPage.columns.hours')}</th>
                <th className="px-4 md:px-6 py-4 text-right">{t('monthlyPage.columns.cost')}</th>
                <th className="px-4 md:px-6 py-4 text-right">{t('monthlyPage.columns.charge')}</th>
                <th className="px-4 md:px-6 py-4 text-center">{t('monthlyPage.columns.status')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
              {filteredJobs.length === 0 ? (
                <tr>
                  <td colSpan={10} className="px-4 md:px-6 py-8 text-center text-gray-500 dark:text-slate-300 text-xl">
                    {t('monthlyPage.emptyDesc')}
                  </td>
                </tr>
              ) : filteredJobs.map((job) => (
                <tr key={job.id} className="hover:bg-gray-50/70 dark:hover:bg-slate-800/60 transition-colors">
                  <td className="px-4 md:px-6 py-4 text-gray-800 dark:text-slate-50">{formatDate(job.date)}</td>
                  <td className="px-4 md:px-6 py-4 font-semibold text-gray-900 dark:text-slate-50">{job.description}</td>
                  <td className="px-4 md:px-6 py-4 text-gray-700 dark:text-slate-200">{job.location || '-'}</td>
                  <td className="px-4 md:px-6 py-4 text-gray-700 dark:text-slate-200">{job.workers?.display_name || job.workers?.alias || '-'}</td>
                  <td className="px-4 md:px-6 py-4 text-gray-700 dark:text-slate-200">{job.job_type || job.type || '-'}</td>
                  <td className="px-4 md:px-6 py-4 text-gray-700 dark:text-slate-200">{job.groups?.name || '-'}</td>
                  <td className="px-4 md:px-6 py-4 text-right text-gray-900 dark:text-slate-50">{job.hours_worked}</td>
                  <td className="px-4 md:px-6 py-4 text-right text-gray-900 dark:text-slate-50">
                    {formatCurrency(job.cost_spent)}
                  </td>
                  <td className="px-4 md:px-6 py-4 text-right font-semibold text-green-700 dark:text-green-300">
